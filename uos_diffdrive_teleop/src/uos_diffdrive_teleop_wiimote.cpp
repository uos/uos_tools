/*This program will run the volksbot in three different speeds
The buttons array shows which buttons are currently depressed on the Wiimtoe device. 
...
double max_vel_x, max_rotational_vel;// declaration of the 
...
int c;// assgning three cases based on three buttons on the wii remote 
if (joy->buttons[0])
c=1;
...
c=3;
//choosing speed and assgning it to max_vel
switch(c)
{
case 1: 
max_vel_x=0.1;
...
//assigning x and z value in the twist.h liabrary 
if  (joy->buttons[8])
...
{
  ros::init(argc, argv, "uos_diffdrive_teleop");

	...
	private_nh_("~")
{
	...
	private_nh_.param("max_vel_x_", max_vel_x_, 0.3);
...
double FreeSpace::calcFreespace(const sensor_msgs::LaserScan::ConstPtr &laserscan){
	...
	for (i = 0; i < laserscan->ranges.size(); i++) {
		...
		orientationWeight = cos(phi/2) * 1.0/(1.0+exp(-(laserscan->ranges[i]-1)/0.3));
	...
	return scanner_orientation_*alpha;
}
...
void FreeSpace::autonomousBehaviour(const sensor_msgs::LaserScan::ConstPtr &laserscan){
	...
	double DTOStopTurning = 1; // defines when to start to turn into freespace
	...
	if (turn_state_ == 1) {
		if ( (distanceToObstacle > DTOStopTurning)) {
			...
			if (omega > 0.0) {
				omega = max(max_rotational_vel_, (max_rotational_vel_ * (1 - (omega / (M_PI / 2)))));
			}
			else if (omega < 0.0) {
				omega = min(-max_rotational_vel_, (max_rotational_vel_ * (-1 - (omega / (M_PI / 2)))));
			}  
	  }
